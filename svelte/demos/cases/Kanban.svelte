<script>
	...
	const refresh = ({ values }) => {
		const ind = cards.findIndex(d => d.id === values.id);
		cards[ind] = values;
	};
</script>

<h4 style="margin-left: 20px;">Imitation of kanban cards</h4>
...
<div class="tasks">
	{#each cards as data}
		...
		<div
			class="item"
			onclick={() => (activeCard = data)}
			class:active={activeCard?.id === data.id}
		>
